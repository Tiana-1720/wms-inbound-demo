...
> **版本**：V1.4.4 | 2026-09-03（§十一 上架列表/焦点托；§十三 装车页呈现与确认交互；取消删除扫描）
> **读者**：研发工程师、测试工程师、产品复核
...
PDA 是加盟揽收仓一线作业的**唯一执行终端**。操作人员通过 PDA 逐箱扫描，在同一操作中完成收货确认与分货绑托；**绑托完成后系统自动生成上架单**（默认一运单一张；**小票混托**时多运单可共一张上架单），操作员基于上架单执行托盘上架至备货库位并产生库位库存；OMS 创建空单调拨计划后，PDA 扫描装车并点「装车」，生成/追加调出仓出库单（**已复核**）；库位库存扣减与调拨计划 **已出库** 由 PC **确认出库**触发（见后台主 PRD §3.4）。
...
- PDA **分货**（收货分货绑托）：菜单 `PDA → 入库管理 → 分货`；**独立新页面**，**不沿用**现网 `PDA → 入库管理 → 绑托`；逐箱扫描箱号；在同一操作中完成收货确认、按**大小票策略**与**作业托数**组托、自动生成**托号**；支持大票多托、小票混托；**无系统满托容量**，由操作员自行判断托满后**确认绑托**；**全部箱绑托完成**时一次性回写 **收货箱数**、**收货人**、**收货时间**、**收货仓库**、**绑托完成时间**；**小票混托**托满绑托时**同事务生成上架单**（多运单共一张）；**大票/小票独托**须运单**全部箱绑托完成**时生成上架单（一运单一张）；**不回写**收货重量/收货体积；**不产生库存变动**（详见 §十二）
...
- PDA **装车**（**空单调拨**路径）：菜单 `PDA → 出库管理 → 装车`；关联**调拨计划单**；扫描库位库存中**可用**（未占用）的已上架货物；**扫箱号带出对应托、关联整托**（Q12）；**每扫一次校验当前托下全部箱**可用库存（R48）；维护司机/车牌/电话（R49）；**不允许拆票装车**（Q15）；点「装车」→ 生成/追加出库单（**已复核**）并**占用**库位库存；**不生**调入仓调拨入库单；头/列表展示 `已扫：N票/M托`（§十三）
- PDA 作业对共用表记录（收货订单/入库单）的状态推进：加盟路径 **待收货 → 已绑托 → 已上架**；**不存在「部分绑托」状态**——运单尚有未绑托箱时头部仍为**待收货**，**全部箱绑托完成**后方→**已绑托**并生成上架单
- PDA 装车对**调拨计划单**：PDA 装车完成后调拨→**已复核**；调拨 **已出库** 由 PC 确认出库触发（非 PDA）
...
- **装车扫描目的仓与调入仓库一致校验**（Q14 装车侧 / R23；创建调拨单仍须填调出/调入仓库，**本期不校验扫描货物目的仓**）
...
| S05 | 空单调拨装车 | **主流程** | 扫描已上架货关联**调拨计划单**；整票装、不拆票；点「装车」→出库单**已复核**、调拨→**已复核** |
...
| 待出库 / 已复核 | PDA 装车 | 已完成必要扫描；维护司机/车牌/电话（弹窗内，R49） | **已复核** | 回写明细；生成/追加出库单→**已复核**；调拨计划→**已复核**；**占用**库位库存 | 拆票/托下箱占用/多托未扫齐：拦截 |
...
| R16 | **PDA 装车**时**占用**库位库存；**PC 确认出库**时库位库存**实扣** |
...
| R24 | **不打印装车清单**（Q17） |
| R25 | PDA 点「装车」：回写明细及司机/车牌/电话；生成/追加出库单→**已复核**；调拨计划→**已复核**；**占用**库位库存；**不生**调入仓调拨入库单 |
...
| R49 | **装车确认交互**：点底部「装车」→ 打开**司机信息弹窗**（司机、电话、车牌必填）→ 点「确认装车」**直接提交**；**无**独立二次确认弹窗 |
...
| `PDA → 出库管理 → 装车` | 装车 | PDA，出库管理 → 装车 | **调拨计划**：**调出仓库**或**调入仓库**命中本仓；仅**空单调拨**待出库 | §5.2.4：装车扫描、装车（§十三） |
...
| V05 | S05 装车 | 已上架；空单待出库 | 扫箱装车（扫箱号带出对应托）→ 点装车 → 司机弹窗确认 | 出库单/调拨**已复核**；不打印清单 |
...
| 2026-09-03 | V1.4.4 §十一：待上架列表不展示初始库位/状态 Tag；上架明细仅焦点托；§十三：装车头/列表 `已扫：N票/M托`、托下全箱占用（R48）、司机弹窗一次提交（R49）、不提供删除/重置（R50–R51）；§5.2.3/§5.2.4 取消删除扫描 |
...
**与作业托数 N 的关系**：**N**（§12.3）为操作员进入分货页时选择的**作业托数**（1–10），决定单托/多托并行作业方式，**不属于**后台分货参数。
...
```
装车计划列表（待出库空单）→ 进入装车作业页 → 扫描箱号装车 → 点「装车」→ 司机信息弹窗 → 【确认装车】
```

- 装车扫描、装车动作见 §5.2.3、§5.2.4、§6.4（R48–R51）
- **不打印**装车清单（Q17 / R24）
...
| 装车确认 | 点「装车」→ **司机信息弹窗**（司机、电话、车牌必填）→ 点「确认装车」**直接提交**；**无**独立二次确认弹窗（R49）；提交后调拨计划与出库单均为**已复核** |
| 不提供能力 | **不提供**重置本次扫描 |
...
| V35 | 装车 | 已扫描≥1票 | 点装车 → 弹窗填司机信息 → 确认装车 | 出库单/调拨**已复核**；**已占用**库位库存；无二次确认弹窗 |
