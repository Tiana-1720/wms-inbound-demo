...
| S12 | 绑托完成生成上架单 | **主流程** | PDA 确认绑托后，系统同事务生成上架单（**作业单号**=TK+…、**运单号**、**托号**、**件数**、**作业类型**=收货上架）；**初始库位**为空；收货订单→已绑托 |
...
>
> **上架入口权限**（沿用现网）：`入库管理→上架` 受上架菜单权限控制；`库内作业→移库作业→上架 TAB` 受**移库作业**权限控制。两入口进入后均查询/操作**同一上架单**，不因加盟揽收仓而禁用移库 TAB 入口（§11.2）。
...
| V19 | S12 生成上架单 | 绑托确认成功 | 查上架单列表 | 一运单一张；**作业单号**（TK+年月日+5 位）已生成；**运单号**、**托号**、**件数**正确；**作业类型**=收货上架；列表列沿用现网含**初始库位**（值为空）；状态=待上架 |
...
> 本章**只写加盟揽收仓相对现网的增量**；绑托、装车见前文，不重复。
...
| # | 现网规则 | 加盟揽收仓增量 | 实现方式 |
| :--- | :--- | :--- | :--- |
| 1 | 入库单全部绑托且状态=**已收货**后生成**收货上架**作业 | **PDA 确认绑托完成**时同事务生成上架单；对应收货订单→**已绑托**（不进收货中/已收货） | 仓库类型分支 + 生成时机调整 |
