...
> **版本**：V1.5.1 | 2026-09-05（第三次修订：建计划同步生成空出库单）
> **读者**：研发工程师、测试工程师、产品复核
...
| 当前状态 | 动作 | 前置条件 | 结果状态 | 后置影响 | 失败处理 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 待出库 | PDA 装车扫描 | 调拨计划可见；货物在库位库存 | 待出库 / 已复核 | 累计扫描明细；扫箱号带出对应托、关联整托；**新扫明细置顶**（R51） | 未上架/无库存：拦截；托下任一箱被占用：Toast `箱号{箱号}已被{占用方}占用`（R48）；**运单多托未扫齐扫下一票**：拦截（R31） |
...
| 待出库 / 已复核 | PDA 装车锁定 | 已扫实装；可用库存充足；维护司机/车牌/电话（R49） | **已复核** | 更新运单/调拨明细；**出库单→已复核**；**箱库存→已占用**；锁定后须 PC 解锁续装 | 拆票/库存不足：拦截 |
...
| R48 | **托下全箱占用校验**：每扫 1 次装车，须校验**当前托下全部箱**在库位库存中可用；任一箱已被其他单据占用 → Toast：`箱号{箱号}已被{占用方}占用` |
| R49 | **装车锁定确认交互**：点「装车锁定」→ **司机信息弹窗**（司机、电话、车牌必填）→ 点「确认装车锁定」提交；「装车暂存」仅保存进度 |
| R50 | 装车页卡片提供「**移除**」按整托删除已扫运单（混托多票一并移除）；**不提供**重置本次扫描；未提交装车前须保持已扫明细，退出再进可继续作业 |
| R51 | 装车头信息/计划列表展示 `已扫：N票/M托`（票数=运单条数，托数=去重托号）；**不展示**调拨状态 Tag、出库单状态、汇总箱数；新扫运单**置顶**入明细 |
...
| 扫描明细 | 新扫运单**置顶**；进入作业页**带出**未提交装车前的已扫明细；卡片「**移除**」按整托删除（R50） |
| 托下占用 | 每扫 1 次校验**当前托下全部箱**可用；任一箱被占用 → Toast：`箱号{箱号}已被{占用方}占用`（R48） |
| 装车确认 | 点「**装车锁定**」→ **司机信息弹窗**（司机、电话、车牌必填）→ 点「确认装车锁定」提交；「**装车暂存**」仅保存进度不锁单（R49） |
| 不提供能力 | **不提供**重置本次扫描 |
