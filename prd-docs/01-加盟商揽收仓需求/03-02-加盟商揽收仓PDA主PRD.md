...
| S03 | 小票混托 / 大票多托 | **主流程** | 小票**同票同托**，多票可混一托（**不设**系统混托票数上限）；大票可拆多托，**无托数上限**（Q18） |
...
| R07 | **大小票组托**见 §12.2；大票**同托同票**；小票**同票同托、多票可混托**；大票多托**无上限**（Q18） |
...
| R37 | **小票判定**：单运单**预报箱数** ≤ **N** 即为小票，否则为大票；**N** 为库内**必填**配置项（数据库维护，**须实施配置后方可分货**；不设系统默认值） |
| R38 | **大票组托**：**同托同票**，一托仅允许一票货；同一运单可拆多托 |
| R39 | **小票组托**：**同票同托**（同一运单箱号不得跨托）；**多票可混一托**；**不设**系统混托票数上限与默认值 |
| R40 | **无系统满托容量配置**；是否托满由操作员现场判断，通过**确认绑托**提交当前托 |
| R41 | **作业托数**：进入分货页设置并行作业格数 **N**（默认 **1**，范围 **1–10**）；**N=1** 为单托单分，**N>1** 为多托同分；**未退出应用前**保持该托数；托满由操作员**确认绑托**，系统不自动满托、不自动绑托 |
| R42 | **N=1·单托单分**：展示 **1** 个作业格；逐箱扫描累计已扫箱数与箱明细；小票允许多票混扫同一格；大票同格仅一票，须**确认绑托**后方可开始该票下一托 |
| R44 | **N>1·多托同分**：展示 **N** 个作业格；**新运单**（含小票）首次扫描分配**最小空闲格**，**一运单一格**，不自动混入他格；已出现运单续扫落入原格；**N** 格均有未绑托货时须先**确认绑托**释放一格；大票同票多托绑托后占用新释放格续扫 |
...
| 2026-09-02 | V1.3.0 需求重确认：PDA「装车」替代「发车」；生成/追加出库单**已复核**；库位库存改由 PC 确认出库扣减；不生调拨入库单；扫箱号带出对应托 |
...
| 3 | 目标库位：关联出库单时仅可选备货区，否则存储区；支持扫描库位带出 | **仅人工选择揽收仓备货库位**（Q10）；**不关联出库单**、不做存储区上架 | 库位选择器过滤 + 去掉出库单联动 |
...
**N** 为库内**必填**配置项；PDA 按运单**预报箱数**与 **N** 实时判定大小票，不在页面硬编码。**小票混托不设 M 参数**，无系统混托票数上限与默认值。

| 类型 | 判定 | 组托规则 |
| :--- | :--- | :--- |
| **小票** | 单运单**预报箱数** ≤ **N** | **同票同托**（同一运单箱号不得跨托）；**多票可混一托** |
| **大票** | 单运单**预报箱数** > **N** | **同托同票**（一托仅一票，**不可混托**）；同一运单可拆**多托**（Q18 无上限） |

**配置项说明**：

| 参数 | 含义 | 维护 |
| :--- | :--- | :--- |
| **N** | 小票阈值（件）：预报箱数 ≤ N 为小票 | 数据库**必填**配置；**须配置后方可分货**；不设系统默认值 |

**按托绑托与头部状态**（R43）：

- **按托确认绑托**后，若运单**尚有未绑托箱号** → 头部**状态**仍为**待收货**；仅回写该托箱明细**托号**
- 运单**全部箱号**绑托完成 → 头部→**已绑托**；一次性回写**收货箱数**等（R03）并**生成上架单**（R33）

> **无满托配置**：系统**不配置**单托最大箱数/重量等满托阈值；是否托满由操作员现场判断，通过**确认绑托**提交当前托（R40）。

**扫描分配原则（呈现层）**：

- 小票：若该运单已在某未绑托托盘中，后续箱号**必须**落入同一托
- 大票：目标托已有其他运单 → 拦截（不可混托）
- 违反规则时 Toast 拦截，文案由 Demo PRD 定义（如「大票不可混托」「小票须在同一托」）
...
| N | 分货方式 | 作业方式 |
| :--- | :--- | :--- |
| **1** | **单托单分** | 展示 **1** 个作业格；逐箱扫描累计已扫箱数与箱明细；**小票允许多票混扫同一格**；大票同格仅一票，须**确认绑托**后方可开始该票下一托 |
| **>1** | **多托同分** | 展示 **N** 个作业格并行作业；**新运单各占一格**（含小票），续扫回原格；见下文格子分配规则 |

**格子分配（N>1，呈现层）**：

1. 运单已在某未绑托格中有箱 → 后续箱**必须**落入该格（同票同托）
2. **新运单**（含小票）→ 分配**最小空闲格**；**一运单一格**，**不**自动落入其他运单所在格（小票混托仅 **N=1** 时适用，见上表）
3. 大票新运单 → 同上，且目标格不得含其他运单
4. **N** 格均有未绑托货 → 须先对任一格**确认绑托**释放后再扫新运单

**大票同票多托（N≥1）**：按托**确认绑托**后，该票续扫占用新释放格或当前空格。
...
```
顶栏「分货」→ [作业托数弹窗] → 扫描箱号 → 运单信息区 → N 个作业格（格内【托满】）
  → 扫描区【重置扫描】（有数据时）
```

- 扫描成功：当前箱高亮，累加对应托的已扫件数
- **重置**：仅清空本页未提交的扫描缓存，不改服务端状态
- **确认绑托**：当前托至少扫入 1 箱；提交该托绑托结果；运单未全部绑托完成时头部保持**待收货**（R43）；全部箱绑托完成时触发 §5.1.3 整单回写并生成上架单；件数与**预报箱数**不一致时提示走上游取消重发（R08），不改预报
...
| V30 | 大小票 | 预报箱数=N 与 N+1 两票分别扫描组托 | 小票可混托；大票不可混托 |
| V31 | 作业托数=1 | 大票多托：扫一托→绑托→再扫下一托 | 未绑托前不可并行第二托 |
| V32 | 作业托数>1 | 设置 N=2，两票小票各扫首箱 | 各占一格；N 格占满须绑托释放 |
