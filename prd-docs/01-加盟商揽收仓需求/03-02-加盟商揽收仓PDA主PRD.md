...
| R57 | 装车头信息/计划列表展示 `已扫：N票/M托`（票数=运单条数，托数=去重托号）；**不展示**调拨状态 Tag、出库单状态、汇总箱数；新扫运单**置顶**入明细 |
...
- 调拨计划 **已复核** 且出库单 **已复核** 时，可再次进入装车追加运单
...
| 扫描明细 | 新扫运单**置顶**；进入作业页**带出**未提交装车前的已扫明细；卡片「**移除**」按整托删除（R56） |
| 托下占用 | 每扫 1 次校验**当前托下全部箱**可用；任一箱被占用 → Toast：`箱号{箱号}已被{占用方}占用`（R54） |
| 装车确认 | 点「**装车锁定**」→ **司机信息弹窗**（司机、电话、车牌必填）→ 点「确认装车锁定」提交；「**装车暂存**」仅保存进度不锁单（R55） |
| 不提供能力 | **不提供**重置本次扫描 |
