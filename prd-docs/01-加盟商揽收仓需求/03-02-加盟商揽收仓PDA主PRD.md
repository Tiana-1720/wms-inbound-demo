...
> **版本**：V1.4.1 | 2026-09-02（§12.2 补充分货参数 M/P 作用说明）
> **读者**：研发工程师、测试工程师、产品复核
...
- PDA **待上架任务列表**：展示本仓待执行的上架单；列表列**沿用现网**（**作业单号**、**运单号**、**托号**、**件数**、**初始库位**）；**运单号**多票时用 `、` 连接；加盟路径**初始库位**值为空，**仍展示该列**
- PDA **装车**（**空单调拨**路径）：菜单 `PDA → 出库管理 → 装车`；关联**调拨计划单**；扫描库位库存中**可用**（未占用）的已上架货物；**扫箱号带出对应托、关联整托**（Q12）；维护司机/车牌/电话；**不允许拆票装车**（Q15）；点「装车」→ 生成/追加出库单（**已复核**）并**占用**库位库存；**不生**调入仓调拨入库单（详见 §十三）
...
| 待出库 / 已复核 | PDA 删除扫描 | 待出库或已复核；已有该运单扫描明细 | 不变 | 从本次实装清单**移除整票运单行** | — |
| 待出库 / 已复核 | PDA 装车 | 已完成必要扫描；维护司机/车牌/电话；目的仓与**调入仓库**一致 | **已复核** | 回写明细；生成/追加出库单→**已复核**；调拨→**已复核**；**占用**库位库存 | 拆票/错装/已占用：拦截 |
...
| V05 | S05 装车 | 已上架；空单待出库 | 扫箱装车（扫箱号带出对应托）→装车 | 出库单/调拨**已复核**；不打印清单 |
...
| V35 | 装车 | 已扫描≥1票 | 点装车 | 出库单/调拨**已复核**；**已占用**库位库存 |
