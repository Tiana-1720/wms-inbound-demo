...
> **版本**：V1.5.2 | 2026-09-07（装车规则 R48–R51 重编号为 R54–R57，与 03-01 去冲突）
> **读者**：研发工程师、测试工程师、产品复核
...
PDA 是揽收仓及相关仓库一线作业的**主要执行终端**。操作人员通过 PDA 逐箱扫描完成分货绑托；**绑托完成后系统自动生成上架单**，操作员基于上架单执行托盘上架至**备货库位**并产生库位库存；OMS 创建空单调拨计划后，PDA 扫描装车并通过「**装车暂存**」/「**装车锁定**」回写实装；库位库存实扣与调拨计划 **已出库** 由 PC **先打印装柜单再确认出库**触发（见后台主 PRD §3.4）。
...
- PDA **分货**：`入库管理 → 分货`；**全仓可用**；**作业托数 N**（**1~99，默认 5**）；支持**多 PDA 并行**；大小票混托、绑托回写详见 §十二
- PDA **上架**：上架至**备货库位**；**加盟揽收仓不测量可上架**；**非加盟揽收仓须测量完成后才能上架**（§11.3 #8）；上架单查询与回写见 §十一
- PDA **待上架任务列表**：展示本仓待执行的上架单；列表展示**作业单号**、**运单号**、**托号**、**件数**；**运单号**多票时用 `、` 连接；**不展示****初始库位**列与**状态** Tag（§11.2、§11.3 #2）
- PDA **装车**（**空单调拨**）：`出库管理 → 装车`；按**运单顺序**、**不可拆票**；「**装车暂存**」/「**装车锁定**」；提交前校验**可用库存**，保存后**箱库存→已占用**；详见 §十三
- PDA 装车**锁定**后调拨→**已复核**；调拨 **已出库** 由 PC **先打印装柜单再确认出库**触发（非 PDA）
...
- 打印装柜单（Q17：PDA **不打印**；由 PC **先打印装柜单再确认出库**）
- **PDA 库存查询**（本期不做；预报/库位库存口径见 §6.3，无独立查询菜单）
- 少件处理、多 PDA 交叉分货（`context/02` 第二次会议 §六）
- **装车扫描目的仓与调入仓库一致校验**（Q14 装车侧 / R23；**本期不做**）
...
| 生成规则 | **默认一运单一张上架单**（对应一入库单共用表记录）。**小票混托例外**：同一托混放多票小票时，**托满确认绑托**即生成**一张**上架单，含托上**全部运单**；PDA 按托上架扫托上任一箱，**整托全部运单**一并完成 |
| 继承字段 | 系统带出并存储：**运单号**（多票 `、` 连接展示）、**托号**、**件数**；**作业类型**=收货上架；**初始库位**为空 |
...
| PDA 职责 | 扫描实装、维护司机/车牌/电话、回写明细（**运单号**、**客户代码**、**箱数**、**重量**、**体积**）；「**装车暂存**」将运单关联至**空出库单**（仍为待出库）或「**装车锁定**」将出库单→**已复核** |
...
| 收货订单 : 上架单 | **默认 1:1**（一运单一张）；**小票混托**时多收货订单可**共用一张**上架单（按托生成，见 R33） |
| 收货订单 : 箱明细 | 1:N，明细按箱号；绑托时关联**托号** |
| 空单调拨 : 出库单 | **建计划时 1:1 同步生成**空出库单（**待出库**、明细空）；装车暂存/关联运单关联明细（仍为待出库）；装车锁定→**已复核** |
...
| S12 | 绑托完成生成上架单 | **主流程** | **大票/小票独托**：运单**全部箱号**绑托完成、收货订单→**已绑托**时同事务生成上架单（一运单一张）。**小票混托**：托满确认绑托时生成**一张**上架单（含托上全部运单） |
...
| S05 | 空单调拨装车 | **主流程** | 扫描已上架货关联**调拨计划单**；整票装、不拆票；**装车暂存**或**装车锁定**→出库单**已复核**、调拨→**已复核** |
...
| S09 | 装车拆票 | **异常** | 同一**运单号**须整票关联本次调拨（Q15） |
...
| 待收货 | PDA 确认绑托（整单完成） | 箱号匹配预报；该运单**全部箱号**已绑托 | 已绑托 | 回写**托号**、**绑托完成时间**、**收货箱数**、**收货人**、**收货时间**、**收货仓库**；**不回写**收货重量/收货体积；**无库存变动**；**同事务生成上架单**（待上架；独托一运单一张，混托见 R33） | 无预报箱号：拦截；件数差异：提示走取消重发 |
...
状态枚举沿用现网：**待出库、已下架、已复核、已出库、已完成、已取消**（与后台主 PRD §5.2 一致）。**空单调拨** PDA 装车路径涉及 **待出库 → 已复核**；**已下架**为计划调拨 PDA 下架态，空单调拨不经过。
...
| 已下架 | **计划调拨** PDA 下架完成（空单调拨不涉及） | 否 |
| 已复核 | PDA 装车完成；出库单未确认出库前可多次装车追加 | 否 |
| 已出库 | PC 确认出库完成（非 PDA 触发） | 否 |
| 已完成 | 东莞对该计划全部关联运单常规入库均已收货完成 | **是** |
| 已取消 | 取消（预留） | **是** |

#### 5.2.2 状态机图（空单调拨 · PDA 视角）

```mermaid
stateDiagram-v2
    [*] --> 待出库 : OMS 创建空白单（非 PDA）
    待出库 --> 待出库 : PDA装车暂存/关联运单
    待出库 --> 已复核 : PDA装车锁定
    已复核 --> 已复核 : PDA装车暂存/追加
    已复核 --> 已出库 : PC确认出库（非 PDA）
...
| 当前状态 | 动作 | 前置条件 | 结果状态 | 后置影响 | 失败处理 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 待出库 | PDA 装车扫描 | 调拨计划可见；货物在库位库存 | 待出库 / 已复核 | 累计扫描明细；扫箱号带出对应托、关联整托；**新扫明细置顶**（R57） | 未上架/无库存：拦截；托下任一箱被占用：Toast `箱号{箱号}已被{占用方}占用`（R54）；**运单多托未扫齐扫下一票**：拦截（R31） |
| 待出库 / 已复核 | PDA 删除扫描 | 待出库或已复核；出库单未确认出库；已有该运单扫描明细 | 不变 | 从本次实装清单**移除整票运单行**（按整托移除，混托多票一并移除） | — |
| 待出库 / 已复核 | PDA 装车暂存 | 已扫实装；可用库存充足 | 待出库 / 已复核 | 将运单**关联至空出库单**并回写明细；保存进度；**箱库存→已占用**；**不锁单据**（出库单仍为待出库） | 可用库存不足：拒绝 |
| 待出库 / 已复核 | PDA 装车锁定 | 已扫实装；可用库存充足；维护司机/车牌/电话（R55） | **已复核** | 更新运单/调拨明细；**出库单→已复核**；**箱库存→已占用**；锁定后须 PC 解锁续装 | 拆票/库存不足：拦截 |
| 已出库 | 东莞收货完成 | 系统回写（非 PDA） | 已完成 | 调拨只读 | — |

#### 5.2.4 动作能力矩阵（调拨计划单 · PDA）

| 动作 | 待出库 | 已复核 | 已下架 | 已出库 | 已完成 | 已取消 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| 查看调拨计划 | ✅ | ✅ | ✅（只读） | ✅（只读） | ✅（只读） | ✅（只读） |
| 装车扫描 | ✅ | ✅（追加） | ❌ | ❌ | ❌ | ❌ |
| 删除扫描 | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
...
| 待上架 | PDA 上架确认 | 已选**备货库位**；**加盟揽收仓**或（**非加盟**且运单**测量已完成**） | 已完成 | 回写**目标库位**、**件数**；联动收货订单→已上架；**预报库存 −**、**库位库存 +** | 未选库位/未测量（非加盟）：不可提交 |
...
| R45 | **小票混托确认绑托**：格内每一票**小票**的已扫箱数须 **= 该票预报箱数**，否则拦截提交 |
| R46 | **扫描落托反馈**：每次扫描成功后弹窗展示**目标作业格/托号**；用户按**扫描键**（或【确认】）关闭；**语音播报**为增强项 |
| R47 | **多托同分（N>1）落托**：小票**首箱**优先入**已有小票且未达 P 上限**的混托格；大票**首箱**占**最小空闲格**（新托） |
| R08 | 预报与实收不一致：PDA **不修改预报**；须在待收货协调上游取消后全量重发 |
| R33 | **上架单生成**（不手工创建）：① **大票/小票独托**：运单**全部箱绑托完成**时同事务生成，**一运单一张**；② **小票混托**：托满确认绑托时生成**一张**上架单，含该托**全部运单**（R45 保证各票已扫齐） |
| R34 | 上架单继承绑托结果：**运单号**（多票 `、` 连接）、**件数**、**作业类型**=收货上架；**初始库位**生成时**为空** |
| R37 | **小票判定**：单运单**预报箱数** ≤ **M** 即为小票，否则为大票；**M** 为库内**必填**配置项（**须配置后方可分货**；不设系统默认值） |
| R38 | **大票组托**：**同托同票**，一托仅允许一票货；同一运单可拆多托 |
| R39 | **小票组托**：**同票同托**；**多票可混一托**，混托格小票运单数 **≤ P** |
| R40 | **无系统满托容量**；**确认绑托**前须满足 §12.2 提交校验（R45） |
| R41 | **作业托数**：进入分货页设置 **N**（默认 **5**，范围 **1–99**）；支持**多 PDA 并行**（各管自己的 N 托）；托满由操作员**确认绑托** |
...
| R10 | **目标库位人工选择**（Q10）；**仅允许备货库位**（库位类型=备货）；写入上架单**目标库位** |
...
| R36 | 大票多托：**一运单一张上架单**；**件数**=该运单全部已绑托箱数；PDA 按托逐托确认至整单完成。**小票混托**：**一托一张上架单**、含多运单；确认上架时**整托全部运单**一并完成 |
...
| R14 | PDA 按托绑托：**库存不变**；**大票/小票独托**须运单全部箱绑托完成时同事务**生成上架单**（R33）；**小票混托**托满绑托时生成上架单 |
| R15 | PDA 上架（上架单完成）：**预报库存 −**、**库位库存 +**；**目标库位仅允许备货库位**（§11.3 #3） |
| R16 | **装车暂存/装车锁定**保存时校验**可用库存**并占用**箱库存**；**PC 确认出库**时库位库存**实扣** |
...
| R23 | 扫描校验货物目的仓与**调入仓库**一致（Q14 装车侧） | **本期不做**（Out of Scope §二）；后续迭代 |
| R24 | PDA **不打印**装柜单（Q17）；PC **先打印装柜单再确认出库** |
| R25 | PDA **装车暂存**：将运单**关联至空出库单**并回写明细；保存进度，**不锁单据**（出库单仍为待出库），占用箱库存。**装车锁定**：更新运单/调拨明细；**出库单→已复核**；调拨→**已复核**；占用箱库存；锁定后须 PC 解锁续装 |
| R52 | **加盟揽收仓不测量可上架**；**非加盟揽收仓须测量完成后才能上架** |
| R53 | **分货**功能**全仓可用**（不限加盟揽收仓）；M/P 按仓配置 |
...
| R30 | 同一调拨 1:1 出库单（**建计划时同步生成**，待出库）；**已复核**前可多次装车追加运单；确认出库后不可再装 |
| R31 | **运单多托顺序**：同一**运单号**在库位库存中若对应**多托**，须**扫齐该运单全部托**后方可扫描**下一运单**的托；混托格一扫关联托上全部运单，该托扫入后这些运单在该托维度视为已扫 |
| R54 | **托下全箱占用校验**：每扫 1 次装车，须校验**当前托下全部箱**在库位库存中可用；任一箱已被其他单据占用 → Toast：`箱号{箱号}已被{占用方}占用` |
| R55 | **装车锁定确认交互**：点「装车锁定」→ **司机信息弹窗**（司机、电话、车牌必填）→ 点「确认装车锁定」提交；「装车暂存」仅保存进度 |
| R56 | 装车页卡片提供「**移除**」按整托删除已扫运单（混托多票一并移除）；**不提供**重置本次扫描；未提交装车前须保持已扫明细，退出再进可继续作业 |
| R57 | 装车头信息/计划列表展示 `已扫：N票/M托`；明细每票展示 **已扫箱数/运单总箱数**；**不展示**客户代码、调拨状态 Tag、出库单状态、汇总箱数；新扫运单**置顶**入明细 |
...
| `PDA → 出库管理 → 装车` | 装车 | PDA，出库管理 → 装车 | **调拨计划**：**调出仓库**或**调入仓库**命中本仓；仅**空单调拨**待出库 | §5.2.4：装车扫描、装车暂存/装车锁定（§十三） |
...
| :--- | :--- | :--- |
| 加盟揽收仓操作员 | 见 §7.1 各菜单数据范围 | 分货、上架、装车扫描、装车 |
...
| 无预报扫描（多货无单） | 绑托扫描拦截 |
| 目的仓与调入仓不一致（装车扫描） | **本期不做**（R23 Out of Scope）；后续迭代 |
...
| :--- | :--- | :--- | :--- | :--- |
| V01 | S01 完整链路 | 待收货预报就绪；已上架库位有货；空单待出库 | 绑托→上架单上架→装车扫描→装车→PC确认出库 | 生成上架单；收货订单→已上架；装车后出库单/调拨**已复核**；确认出库后库位库存扣减 |
| V02 | S02 大票绑托 | 大票预报就绪 | 作业托数=1 逐箱扫描并确认绑托 | 同托仅该票；可多托；状态→已绑托；**生成上架单**；库存不变 |
| V19 | S12 生成上架单 | 大票全部绑托完成；或小票混托托满绑托 | 查上架单列表 | 独托：**一运单一张**；混托：**一托一张**、**运单号**多票 `、` 连接；**作业单号**（TK+年月日+5 位）已生成；**托号**、**件数**正确；**作业类型**=收货上架；状态=待上架 |
| V03 | S03 小票混托 | 多小票预报就绪 | 同托混扫多票小票并绑托 | 绑托成功；各票**收货箱数**正确；同票不跨托 |
| V30 | 大小票判定 | 运单预报箱数分别为 M、M+1 | 扫描分货 | ≤M 走小票规则；>M 走大票规则 |
...
| V05 | S05 装车 | 已上架；空单待出库 | 扫箱装车→**装车暂存**或**装车锁定**（锁定须司机弹窗） | 暂存：不锁单、箱库存已占用；锁定：出库单/调拨**已复核**；PDA 不打印装柜单 |
| V06 | 托下箱占用 | 托上某箱已被其他计划占用 | 扫描该托任一箱 | Toast：`箱号{箱号}已被{占用方}占用`；整托不入明细 |
...
| V10 | S09 错装拦截（目的仓≠调入仓库） | — | — | **本期不做**（R23 Out of Scope） |
...
| 2026-09-03 | V1.4.2 **小票混托上架单**：多运单可共一张上架单；托满绑托即生成；按托上架整托全部运单一并完成；R33/R36/R14 同步 |
| 2026-09-03 | V1.4.3 **装车目的仓校验（R23）本期不做**；创建调拨单仍须填调入仓库（Q14 计划侧）；S09 仅保留拆票（Q15） |
| 2026-09-05 | V1.5.1 第三次修订：建计划同步生成空出库单（待出库）；装车暂存关联运单至出库单 |
| 2026-09-07 | V1.5.2 装车规则重编号：**R48–R51** → **R54–R57**，与 `03-01` R48–R51（M/P、备货库位、PC 关联运单、打印装柜单）去冲突 |
...
| **初始库位** | 展示设备号+分货口或「人工收货」 | **为空**；作业页**不展示**；待上架列表**不展示**初始库位列与状态 Tag（§11.2、§11.3 #2） |
...
| 点击已上架/未上架箱/托数→**上架明细**子页（全部/已上架/未上架 TAB，展示托号、箱号、库位） | ✅ 沿用（R36 按托逐托确认）；加盟路径**仅展示当前焦点托**明细（§11.3 #7） |
| 点击【上架】后，**当前托号下全部货箱**变为已上架；全部完成后单据完结 | ✅ 沿用 |
| **作业单号**编码 `TK`+年月日+5 位流水号 | ✅ 沿用 |
| **作业类型**=收货上架 | ✅ 沿用 |
| 上架库存结转：**预报库存− / 库位库存+** | ✅ 沿用，**无调整**（R15） |
| 待上架列表列 | ✅ 沿用现网列骨架；加盟路径**不展示****初始库位**列与**状态** Tag，仅展示**作业单号**、**运单号**、**托号**、**件数** |

### 11.3 加盟揽收仓增量（需在现网页面上调整）

| # | 现网规则 | 加盟揽收仓增量 | 实现方式 |
| :--- | :--- | :--- | :--- |
| 1 | 入库单全部绑托且状态=**已收货**后生成**收货上架**作业 | **大票/小票独托**：收货订单**全部箱绑托完成**、状态→**已绑托**时**同事务**生成上架单（一运单一张）。**小票混托**：托满绑托时生成一张含多运单的上架单 | 仓库类型分支 + 生成时机调整 |
| 2 | **初始库位**展示设备号+分货口或「人工收货」 | **初始库位为空**（无设备/分货口）；上架作业页**不展示**初始库位；待上架列表**不展示**初始库位列与**状态** Tag | 作业页字段隐藏；列表列裁剪 |
...
| 7 | 上架明细展示全部托 | 上架作业页与上架明细子页**仅展示当前焦点托**（扫描或列表进入时确定的托）；切换托须回到作业页扫描/切换焦点 | 明细 TAB 按焦点托过滤 |
| 8 | 测量前置 | **加盟揽收仓**：**不测量可上架**。**非加盟揽收仓**：运单**测量完成后**才能上架 | 按仓库类型分支校验 |
...
进入后（两入口合一）
  ...
  └─ 方式 B：待上架任务列表
        → 展示：**作业单号**、**运单号**、**托号**、**件数**（**不展示**初始库位、状态 Tag）
        → 点击进入上架作业页
...
上架作业页（沿用现网骨架）
  ...
  └─ 顶部进入上架明细（全部/已上架/未上架 TAB；**仅当前焦点托**）
```
...
| V25 | 明细 TAB | 多托作业中 | 打开上架明细 | **仅展示当前焦点托**的箱号、库位；全部/已上架/未上架 TAB 在焦点托维度过滤 |
...
> **菜单路径**：`PDA → 入库管理 → 分货`  
> **适用范围**：**全仓可用**（加盟揽收仓、自营揽收仓、集货仓等）；**不沿用**现网 `绑托` 页
...
| 核心职责 | 逐箱扫描，按大小票策略与作业托数组托；操作员确认托满后绑托；**独托**运单全部箱绑托完成后生成上架单；**小票混托**托满绑托时生成上架单 |
...
**分货参数**由管理后台按**加盟揽收仓**维度维护（维护入口与权限见《03-01》§3.5），是 PDA 分货页判定**大小票**、控制**小票混托**的规则源头。PDA **只读取、不维护**；**M**、**P** 均未配置时，分货入口提示「请先配置分货参数」，不可作业。

| 参数 | 作用（PDA 侧） |
| :--- | :--- |
| **M** | 按运单**预报箱数**实时判定大小票：≤ M 为小票，> M 为大票；大小票决定能否混托、绑托校验是否要求扫齐全箱 |
| **P** | 限制同一未绑托混托格内最多容纳的**小票运单数**；达上限后须先**确认绑托**释放格位，或扫入其他可用格 |

**与作业托数 N 的关系**：**N**（§12.3）为操作员进入分货页时选择的**作业托数**（**1–99**，默认 **5**），决定单托/多托并行作业方式，**不属于**后台分货参数。

**符号约定**：**M**=小票阈值；**P**=小票混托票数上限；**N**=作业托数（§12.3）。PDA 按运单**预报箱数**与 **M** 实时判定大小票。
...
**后台配置项**（管理后台维护，见《03-01》§3.5）：

| 参数 | 含义 | 维护与缺省 |
| :--- | :--- | :--- |
| **M** | 小票阈值（件）：预报箱数 ≤ M 为小票 | **必填**；未配置则 PDA 分货不可用 |
| **P** | 小票混托票数上限：同一未绑托混托格内最多容纳的小票运单数 | **必填**；按加盟揽收仓维度配置；建议默认 **5** |
...
进入分货页设置**作业托数 N**（默认 **5**，范围 **1–99**，R41）；**N** 写入终端本地会话，**退出应用前**保持。支持**多 PDA 并行**（各管自己的 N 托，单台完成才能提交，**全部绑完才能上架**）；**暂不考虑交叉作业**。
...
```
装车计划列表（待出库空单）→ 进入装车作业页 → 扫描箱号装车 → 「装车暂存」或「装车锁定」→（锁定）司机信息弹窗 → 【确认装车锁定】
```

- **装车暂存**：保存部分装车进度，**不锁单据**；提交前校验**可用库存**；保存后**箱库存→已占用**
- **装车锁定**：确认本批装车完成；提交前校验**可用库存**；保存后**箱库存→已占用**；出库单/调拨→**已复核**；锁定后须 **PC 解锁**才能续装（提交时提示）
- 装车扫描规则见 §5.2.3、§6.4（R54–R57）
- PDA **不打印**装柜单（Q17 / R24）；PC **先打印装柜单再确认出库**
- 调拨计划 **已出库 / 已完成**：装车页**只读**，无二次装车入口（R32）
- 装车列表：关联出库单**待出库或已复核**且**是否锁单=否**；**已锁单**不展示
- 出库单 **已复核** 且 **是否锁单=否** 时（如装车暂存后），可再次进入装车追加运单

### 13.2 页面呈现与交互（加盟增量）

| 区域 | 规则 |
| :--- | :--- |
| 计划列表 / 作业页头 | 展示 `已扫：N票 / M托`（票数=运单条数，托数=去重托号）；**不展示**调拨状态 Tag、出库单状态 Tag、汇总箱数 |
| 扫描明细 | 新扫运单**置顶**；每票展示 **已扫箱数/运单总箱数**；**不展示客户代码**（后台仍回写）；进入作业页**带出**未提交装车前的已扫明细；卡片「**移除**」按整托删除（R56） |
| 托下占用 | 每扫 1 次校验**当前托下全部箱**可用；任一箱被占用 → Toast：`箱号{箱号}已被{占用方}占用`（R54） |
| 装车确认 | 底部 **装车锁定**（左，次要）/ **装车暂存**（右，主色）；点「**装车锁定**」→ **司机信息弹窗**（司机、电话、车牌必填）→ 点「确认装车锁定」提交；「**装车暂存**」仅保存进度不锁单（R55） |
| 不提供能力 | **不提供**重置本次扫描 |
...
| V35 | 装车锁定 | 已扫描≥1票；可用库存充足 | 点装车锁定 → 弹窗填司机信息 → 确认 | 出库单/调拨**已复核**；**箱库存已占用** |
| V35a | 装车暂存 | 部分装车 | 点装车暂存 | 保存进度；**不锁单据**；**箱库存已占用** |
