...
function detectAnnotationBasePath() {
  const script = document.querySelector('script[src*="annotation-kit/runtime"]');
  if (!script) return '';
  try {
    const scriptPath = new URL(script.getAttribute('src'), location.href).pathname;
    const match = scriptPath.match(/^(.*)\/annotation-kit\/runtime\.js$/);
    return match?.[1] || '';
  } catch (error) {
    return '';
  }
...
function currentRouteCandidates() {
  ...
  const basePath = detectAnnotationBasePath();
  if (basePath && location.pathname.startsWith(basePath)) {
    ...
    try {
      addVariants(decodeURIComponent(stripped));
    } catch (error) {
      // Ignore decode failures for stripped paths.
    }
  }
